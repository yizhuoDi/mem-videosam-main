...
if __name__ == "__main__":
    ...
    for frame_id in range(1, video_lenth+1):
        print("*************Processing frame ", frame_id)
        frame_path = image_path + "%06d.jpg" % frame_id
        ...
        masks, _, _, object_tokens = predictor.predict_torch(
            point_coords=None,
            point_labels=None,
        ...
        if frame_id == 1:
            prev_tokens = object_tokens
            num_object = prev_tokens.shape[0]
            color_list_prev = [np.concatenate([np.random.random(3), np.array([0.6])], axis=0) for _ in range(num_object)]
        indices = match_from_embds(object_tokens, prev_tokens)
        color_list_new = []
    ...
    # frame_one = gif[0]
    # frame_one.save("./gif/video2.gif", format="GIF", append_images=gif,
    #            save_all=True, duration=100, loop=0)
    height, width, _ = gif[0].shape
    size = (width, height)
    # out = cv2.VideoWriter('./output/videosam.avi', cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
    out = cv2.VideoWriter('./output/videosam.avi', 0, 1, (width, height))
