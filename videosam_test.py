...
def EM_loss(cur_feature,pred_feature,attn_index):
    ...
    n_buffer=pred_feature.shape[1]
    loss=torch.tensor([0]).to(float).cuda()
    ...
    for n in range(b):
        ...
        for i in range(n_obj):
            for j in range(n_buffer):
                ...
                index_prob = attn_index[n,i,j].cuda()   
...
if __name__ == "__main__":
    ...
    train_dataset = wds.WebDataset(args.train_data_path,resampled=True).decode("rgb")
    train_dataset=train_dataset.with_epoch(1200)
    #sampler = DistributedSampler(train_dataset) # 这个sampler会自动分配数据到各个gpu上
    ...
    mem=mem.cuda()
    mem=torch.nn.parallel.DistributedDataParallel(mem,find_unused_parameters=True)
    ...
    for epoch in range(args.epoch_num):
        ...
        for batch,batch_data in enumerate(loader):
            ...
            if batch%vis_batchlen==0:
                ...
                frame_one = gif[0]
                frame_one.save(f"./gif/video_{batch}_{epoch}.gif", format="GIF", append_images=gif,
